...
COIN_CHART= os.path.dirname(__file__)
...
def result_graph(fit_history,coin_name):
    ...
    if not os.path.exists(f"{COIN_PATH}/static"):
        os.makedirs(f"{COIN_PATH}/static")
...
def confirm_pred(y_true, y_pred, coin_name):
    ...
    if y_true.shape == y_pred.shape:
        ...
        plt.legend()
        plt.savefig(f"{COIN_PATH}/static/chart/{coin_name.lower()}_scatter.png") #plt.close쓰는게 좀더 좋더라
...
if __name__=="__main__":
    ...
    for coin_name in coin_names:        
        ...
        # print(y_train.shape)
        # print(y_pred.shape)
        confirm_pred(y_train,y_pred,coin_name=coin_name)
