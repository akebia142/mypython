...
#  일자에 따른 예측값 출력함수
def y_predict(model, x_pred, timegap, coin_name):
    global robust_scaler
    data_array= []
    if not robust_scaler:
        scaler_path = os.path.join(COIN_PATH, f"{coin_name.lower()}_robust_scaler")
    ...
    return [arr.tolist() for arr in data_array]
...
if __name__=="__main__":
    ...
    print(x_pred.shape)
    print(x_pred[0])
    lstm_model = load_rnnmodel(
        os.path.join(COIN_PATH, "lstm_model.keras")
    )
    print(lstm_model)
