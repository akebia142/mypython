...
from ai_service.Crypto_Coin_Model import get_data, extract_data
#BTC, ETH, XRP
#코인 파일 prefix btc_,eth_,xrp_
service_coins = ["BTC","ETH","XRP"]
COIN_PATH = os.path.join(os.path.dirname(__file__), "coin_config")
btc_lstm_model = load_rnnmodel(os.path.join(COIN_PATH, "btc_lstm_model.keras"))

eth_lstm_model = load_rnnmodel(os.path.join(COIN_PATH, "eth_lstm_model.keras"))

xrp_lstm_model = load_rnnmodel(os.path.join(COIN_PATH, "xrp_lstm_model.keras"))
btc_err_rate=""
eth_err_rate=""
xrp_err_rate=""
btc_graph=""
...
xrp_graph=""
def init_server():
    global btc_err_rate, eth_err_rate, xrp_err_rate
    global btc_graph, eth_graph, xrp_graph
